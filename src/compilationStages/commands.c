...
int commands(Data_model *data_model, Line_params **line_params, size_t *line_params_count)
{
    ...
    else if (assembler_commands[result].command_type == 1)
    {
        ...
        if ((*line_params)[*line_params_count - 1].param_count != word + 1)
        {
            ...
            if ((ptr_open = strchr((*line_params)[*line_params_count - 1].parsed_params[word], '[')) != NULL)
            {
                /* if last char in word is '[' */
                if (*ptr_open == (*line_params)[*line_params_count - 1].parsed_params[word][strlen((*line_params)[*line_params_count - 1].parsed_params[word]) - 1])
                {
                    /* the index in word+1*/
                    if (strchr((*line_params)[*line_params_count - 1].parsed_params[word + 1], ']') != NULL)
                    {
                        /* index and ] in word+1, opernad end */
                        strcat((*line_params)[*line_params_count - 1].parsed_params[word], (*line_params)[*line_params_count - 1].parsed_params[word + 1]);
                    }
                    else
                    {
                        /* index in word+1, check if "]" is the word+2 */
                        if (strlen((*line_params)[*line_params_count - 1].parsed_params[word + 2]) == 1 && (*line_params)[*line_params_count - 1].parsed_params[word + 2][0] == ']')
                        {
                            /* "]" is the word+2, operand end*/
                            strcat((*line_params)[*line_params_count - 1].parsed_params[word], (*line_params)[*line_params_count - 1].parsed_params[word + 1]);
                            strcat((*line_params)[*line_params_count - 1].parsed_params[word], (*line_params)[*line_params_count - 1].parsed_params[word + 2]);
                        }
                        else
                        {
                            /* error */
                ...
                else
                {
                    ...
                    if (strlen((*line_params)[*line_params_count - 1].parsed_params[word + 1]) == 1 && (*line_params)[*line_params_count - 1].parsed_params[word + 1][0] == ']')
                    {
                        /* operand end */
                        strcat((*line_params)[*line_params_count - 1].parsed_params[word], (*line_params)[*line_params_count - 1].parsed_params[word + 1]);
                    }
        ...
        /* check address type */
        addressing_target = check_addressing(&(*line_params)[*line_params_count - 1].parsed_params[word], data_model);
    ...
    else if (assembler_commands[i].command_type == 2)
    {
        ...
        else if ((*line_params)[*line_params_count - 1].param_count == word + 3) /* comma is a seperate word and must be in word + 1  */
        {
            ...
            addressing_source = check_addressing(&(*line_params)[*line_params_count - 1].parsed_params[word], data_model);
...
int check_addressing(char **word, Data_model *data_model)
{
    ...
    else if (strchr(*word, '[') != NULL)
    {
        ...
        index[length] = '\0';
        operand_target = (*line_params)[*line_params_count - 1].parsed_params[word];
        */
        result = 2;
    }
